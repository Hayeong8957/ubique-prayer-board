...
interface HorebDetailPageProps {
...
export default function HorebDetailPage({
  post,
  canManage,
  currentUserId,
  error,
}: HorebDetailPageProps) {
  ...
  async function onDelete() {
    const ok = window.confirm("이 호렙산 기도회 말씀을 삭제할까요?");
    if (!ok) return;
...
export const getServerSideProps: GetServerSideProps<HorebDetailPageProps> = async (context) => {
  ...
  try {
    ...
    if (!post || post.boardCode !== "horeb") {
      return {
        props: {
          post: null,
          canManage: false,
          currentUserId: session?.user?.id ?? null,
          error: "호렙산 기도회 게시글이 아닙니다.",
        },
      };
    }
    return {
